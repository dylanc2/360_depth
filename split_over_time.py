...
def run_mp4(file_name):
    ...
    np.save('predicted_points.npy', np.array(sample_good_new))
    print("pred", np.asarray(sample_good_new).shape)
...
def plot_3d():
    ...
    for i in range(num_frames):
        ...
        ax.scatter3D(predicted_img_points[1], depths[i][predicted_img_points[1], predicted_img_points[0]], predicted_img_points[0], color=str((i+1)/(num_frames+1)))
...
def plot_kornia():
    ...
    n = []
    for i in range(B):
        a = [[9.51150262*10**3, 0.00000000e+00, 1.87199625*10**3],[0, 9.60017576*10**3, 1.22511258*10**3],[0,0, 1]]
        j = torch.tensor(a)
        n.append(j)
    ...
    for time in range(B):
        ...
        ax.scatter(xs, ys, zs, color = str((time)/(B+10)))
        plt.show()
